...
const Editor = ({ content, setContent, handleSelection, quillRef, isDark, setIsDark }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white relative z-0 transition-colors duration-500">
      ...
      {/* Editor Header */}
      <div className="h-[72px] border-b border-slate-200 px-8 flex items-center justify-between bg-white shrink-0 transition-colors duration-500">
        <div className="flex items-center gap-4">
          ...
          <div>
            <input 
              ...
              className="text-slate-800 font-bold text-[16px] bg-transparent outline-none focus:border-b-2 hover:border-b border-slate-300 w-48 mb-[1px]" 
            />
            <p className="text-teal-600 text-[11px] font-bold tracking-widest uppercase">Saved to Local Storage</p>
          </div>

          <button onClick={() => setIsDark(!isDark)} className="ml-4 p-2.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-all cursor-pointer border border-transparent hover:border-slate-200" title="Toggle Theme">
        ...
        <div className="flex items-center gap-2">
           <button onClick={toggleDictation} className={`flex flex-col items-center justify-center w-11 h-11 rounded-xl transition-all cursor-pointer ${isDictating ? 'bg-rose-100 text-rose-600 animate-pulse border border-rose-200' : 'text-slate-500 hover:bg-slate-100 border border-transparent hover:text-slate-700'}`} title="Voice Typing">
             <Mic className="w-5 h-5" />
           </button>
           <button onClick={toggleReadAloud} className={`flex flex-col items-center justify-center w-11 h-11 rounded-xl transition-all cursor-pointer ${isReading ? 'bg-teal-100 text-teal-600 animate-pulse border border-teal-200' : 'text-slate-500 hover:bg-slate-100 border border-transparent hover:text-slate-700'}`} title="Read Aloud">
             <Volume2 className="w-5 h-5" />
           </button>
           <button onClick={handleMagicFormat} disabled={isFormatting} className="flex items-center gap-2 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-100 border border-transparent font-bold text-[13px] px-4 py-2.5 rounded-xl transition-all cursor-pointer disabled:opacity-50" title="Auto-Format Magic">
             {isFormatting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />} Magic Format
           </button>

           <div className="w-px h-6 bg-slate-200 mx-2"></div>

           <button onClick={handleExportTxt} className="flex items-center gap-2 text-slate-500 hover:text-slate-800 hover:bg-slate-100 font-bold text-[13px] px-4 py-2.5 rounded-xl transition-all cursor-pointer">
             <Download className="w-4 h-4" /> Export TXT
           </button>
           <button onClick={() => setShowPdfModal(true)} className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-[13px] px-5 py-2.5 rounded-xl transition-all shadow-md cursor-pointer">
      ...
      {/* Main Canvas Area */}
      <div className="flex-1 overflow-y-auto px-16 py-8 relative flex justify-center bg-slate-50 border-t border-slate-200 transition-colors duration-500">
        <div className="w-full max-w-[850px] shadow-sm bg-white min-h-[max(calc(100vh-200px),800px)] h-auto relative group flex flex-col pt-2 border border-slate-200 rounded-b-xl overflow-hidden transition-colors duration-500 pb-24">
           <ReactQuill 
             ref={quillRef}
             theme="snow"
             value={content}
      ...
      {/* Live Stats Ribbon */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md px-6 py-3 rounded-full shadow-[0_5px_20px_-3px_rgba(0,0,0,0.1)] border border-slate-200 text-[12px] font-bold text-slate-500 flex items-center justify-center gap-8 z-10 transition-all hover:scale-[1.02]">
         <div className="flex items-center gap-2.5"><span className="w-2 h-2 rounded-full bg-teal-400 shadow-[0_0_8px_rgba(45,212,191,0.6)]"></span> <span className="text-slate-700">{wordCount}</span> Words</div>
         <div className="flex items-center gap-2.5"><span className="w-2 h-2 rounded-full bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.6)]"></span> <span className="text-slate-700">{charCount}</span> Chars</div>
         <div className="flex items-center gap-2.5"><span className="w-2 h-2 rounded-full bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.6)]"></span> <span className="text-slate-700">{readTime}</span> Min Read</div>
      </div>
